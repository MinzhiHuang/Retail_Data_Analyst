...
-- Inconsistent found, further analysis by customer segment
SELECT 
    customer_segment,
...
-- Deep dive: Only orders_count = 0 records have inconsistency
SELECT 
    'Orders count consistency (orders_count > 0)' AS check_name,
    COUNT(*) AS inconsistent_rows,
    ROUND(100.0 * COUNT(*) / (SELECT COUNT(*) FROM customer_transactions WHERE orders_count > 0), 2) AS inconsistent_pct
FROM customer_transactions
WHERE orders_count > 0  
  AND orders_count != (morning_orders + midday_orders + evening_orders);
